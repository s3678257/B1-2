...
var Table = function () {
    ...
    _createClass(Table, [{
        ...
        value: function tableHtml() {
            ...
            var container = ['<div class="Table">', '<div class="T-title"style="overflow: auto;">', '<div>' + title + '</div>', '</div>', '<div class="T-content">', '<label for="T-sort">', 'Number of items displayed per page : <select name="" id="T-pageNum">'].concat(_toConsumableArray([5, 10, 15].map(function (c) {
                return '<option value="' + c + '">' + c + '</option>';
            })), ['</select>', 'Sort by : <select name="" id="T-sort">'], _toConsumableArray(columns.map(function (c) {
                return '<option value="' + c.data + '">' + c.title + '</option>';
            })), ['</select>','Search : <input type="text" name="T-search" placeholder="search" style="height: 40px"></label>', '<table border="0" cellspacing="0" cellpadding="0">'], _container, ['</table>', '<div id="T-page"></div>', '</div>', '</div>']).join('');
        ...
        value: function showDb() {
            ...
            if (searchVal !== "") {
                ...
                cb(data);
                if (data.length === 0) {
                    $('tbody', this.$ele).html('<tr><td colspan="' + columns.length + '">Sorry, no relevant data was found</td></tr>');
